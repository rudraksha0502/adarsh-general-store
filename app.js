...
async function placeOrder() {
  ...
  try {
    ...
    // 2. Saves to Supabase DB (orders table)
    // 3. Saves reference to localStorage (swift_orders)
    // 4. Builds PDF invoice in-memory (jsPDF)
    // 5. Sends PDF to Telegram as document
    // 6. PDF Blob is discarded automatically
    await placeOrderAndNotify(customer, cartSnap, total);
    ...
    return `
      <div class="my-order-card" id="my-order-${escHtml(order.id)}">
        ...
        <div class="my-order-footer">
          ...
          <div class="my-order-total">Total: <strong>₹${Number(order.total).toLocaleString("en-IN")}</strong></div>
        </div>
      </div>`;
  }).join("");
}
  ...
  db.channel("orders-deletes")
    .on(
      ...
      payload => {
        ...
        if (!deletedId) return;
        removeOrderFromLocalStorage(deletedId);
        // If My Orders modal is open, refresh it
        const modal = document.getElementById("my-orders-modal");
        if (modal && modal.style.display !== "none") {
          renderMyOrders();
        }
        showToast(`✅ Order ${deletedId} has been delivered!`);
      }
    )
    .subscribe();
}
