/* ═══════════════════════════════════════════════════════════
   ...
   Changes:
   • MRP + Selling Price fields (separate, no duplicate IDs)
   • Category image upload (stored in Supabase bucket)
   • Shows MRP/discount in product list
   • Passes mrp column to Supabase
═══════════════════════════════════════════════════════════ */
...
const CAT_BUCKET    = "category-images"; // separate bucket for category icons
...
async function uploadCategoryImage(file) {
  ...
  // Try cat bucket first; fall back to product bucket if not found
  try {
    return await uploadToStorage(CAT_BUCKET, file, "uploads", "cat-upload-bar-wrap", "cat-upload-bar");
  } catch (err) {
    if (err.message.includes("not found")) {
      // Fallback: reuse product-images bucket with a cat_ prefix
